package org.openmrs.module.patientqueuapp.web.controller.utils;
...
public class RegistrationWebUtils {
    ...
    public static Encounter createEncounter(Patient patient, boolean revisit) {
        ...
        if (!revisit) {
            String encounterTypeName = GlobalPropertyUtil.getString(PatientQueueConstants.PROPERTY_ENCOUNTER_TYPE_REGINIT,
                    "REGINITIAL");
            encounterType = Context.getEncounterService().getEncounterType(encounterTypeName);
        } else {
            String encounterTypeName = GlobalPropertyUtil.getString(PatientQueueConstants.PROPERTY_ENCOUNTER_TYPE_REVISIT,
                    "REGREVISIT");
            encounterType = Context.getEncounterService().getEncounterType(encounterTypeName);
        }

        // get location
        Location location = new Location(GlobalPropertyUtil.getInteger(PatientQueueConstants.PROPERTY_LOCATION, 1));
